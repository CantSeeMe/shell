...
/*   Updated: 2017/09/24 10:56:21 by jye              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_cdir(t_cdir **n)
{
	...
	if (*n == 0)
	{
		...
		ft_memset((*n)->file, 0, sizeof(char *) * 128);
	}
	z = (*n)->nb_file;
	while (z--)
	{
...
static int	cdir_realloc(t_cdir *n)
{
	...
	i = 0;
	ft_memset(n->file + n->s_alloc, 0, n->s_alloc);
	while (i < n->s_alloc)
	{
		n->file[i] = f[i];
		i++;
...
t_cdir		*buff_get_alike(char *fold, char *alike)
{
	...
	while ((cwf = readdir(cdir->cwd)))
	{
		if (*alike != '.' && cwf->d_name[0] == '.')
			continue ;
		if (ft_strncmp(cwf->d_name, alike, als))
			continue ;
		if (cdir->nb_file >= cdir->s_alloc && cdir_realloc(cdir))
			break ;
		if ((cdir->file[cdir->nb_file] = ft_strdup(cwf->d_name)) == 0)
			continue ;
		if ((zz = ft_strlen(cdir->file[cdir->nb_file])) > cdir->maxlen)
			cdir->maxlen = zz;
		cdir->nb_file++;
	}
	return (cdir);
