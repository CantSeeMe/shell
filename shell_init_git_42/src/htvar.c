...
/*   Updated: 2017/09/24 11:44:20 by jye              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		frag_var(char *s, char **key, char **value)
{
	...
	if ((ptr = ft_strchr(s, '=')) == 0)
		return (1);
	klen = ptr - s;
	*value = ptr[1] == 0 ? 0 : ft_strdup(ptr + 1);
...
char	*defrag_var(char *key, char *value)
{
	...
	ks = ft_strlen(key);
	vs = value ? ft_strlen(value) : 0;
	if ((s = malloc(ks + vs + 2)) == 0)
		return (0);
	ft_memcpy(s, key, ks);
	s[ks] = '=';
	if (value)
		ft_memcpy(s + ks + 1, value, vs);
	s[ks + vs + 1] = 0;
	return (s);
}
