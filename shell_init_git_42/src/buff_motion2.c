...
/*   Updated: 2017/10/01 06:48:58 by jye              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	buff_kill_next(void)
{
	...
	ft_memset(g_buffer.s + g_buffer.cu, ' ', diff);
...
void	buff_kill_prev(void)
{
	...
	ft_memmove(g_buffer.s,
				g_buffer.s + g_buffer.cu,
				diff);
	ft_memset(g_buffer.s + diff, ' ', g_buffer.cu);
...
void	buff_reset_state(void)
{
	...
	{
		l = ft_strlen(chro->s);
		ft_memcpy(g_buffer.s, chro->s, l);
	}
	else
		l = 0;
	ft_memset(g_buffer.s + l, ' ', g_buffer.len - l);
	buff_refresh(0, g_buffer.s, g_buffer.len);
